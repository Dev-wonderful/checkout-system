...
export default function Home() {
  ...
  const handleAddToCart = (product: Product, quantity: Quantity[]) => {
  ...
  const handleRemove =  (item: Product, itemQuantity: Quantity) => {
    console.log('check')
  ...
  const handleClearCart =() => {
    ...
    const defaultQty: Quantity = quantity.map(c => {id: c.id, c.value = 1}); //bug
